...
public class OurMissionPageTest {
    ...
    @Test
    @DisplayName("Скролим и открываем цитату") // БАГ
    public void ScrollAndViewQuote() throws InterruptedException {
        onView(isRoot()).perform(ViewActions.waitElement(withId(R.id.our_mission_image_button), 10000)); // ожидаем появления нужного элемента
        onView(withId(R.id.our_mission_image_button)).perform(click()); // находим иконку для входа и кликаем по ней
        onView(withText("Главное - жить любя")).check(matches(withText("Главное - жить любя"))); // проверяем что отображается заголовок страницы
        onView(ViewMatchers.withId(R.id.our_mission_item_list_recycler_view)).perform(RecyclerViewActions.scrollTo(hasDescendant(withText("Важен каждый!")))); // скролим вниз до последней цитаты
        onView(isRoot()).perform(ViewActions.waitElement((withText("Служение человеку с теплом, любовью и заботой")), 10000)); // ожидаем появление нужного элемента
        onView(withText("Служение человеку с теплом, любовью и заботой")).perform(click()); // кликаем цитате
        Thread.sleep(5000);
        onView(withIndex(withId(R.id.our_mission_item_description_text_view), 4)).check(matches(isDisplayed())); // проверяем что описание цитаты отображается
    }
